...
select	dep.nome "Nome Departamento",
		count(*) "Numero de Empregados",
		coalesce(round(avg(venc.valor), 2), 0) as "Media Salarial",
		coalesce(max(venc.valor), 0) as "Maior Salario",
		coalesce(min(venc.valor), 0) as "Menor Salario"
from empregado emp
left join emp_venc e on emp.matr = e.matr
left join vencimento venc on e.cod_venc = venc.cod_venc
left join divisao divi on emp.lotacao_div = divi.cod_divisao
left join departamento dep on divi.cod_dep = dep.cod_dep
group by dep.nome;


--- cálculo de salários (deve-se considerar descontos - incompleto)

select	emp.matr,
		sum(case when venc.valor is null then 0 else venc.valor end)
from empregado emp
left join emp_venc e on emp.matr = e.matr
left join vencimento venc on e.cod_venc = venc.cod_venc
group by emp.matr;

--- salários por departamento (incompleto)

select	dep.nome "Nome Departamento",
		count(distinct(emp.matr)) "Numero de Empregados",
		round(avg(case when venc.valor is null then 0 else venc.valor end), 2) as "Media Salarial",
		max(case when venc.valor is null then 0 else venc.valor end) as "Maior Salario",
		min(case when venc.valor is null then 0 else venc.valor end) as "Menor Salario"
from empregado emp
left join emp_venc e on emp.matr = e.matr
left join vencimento venc on e.cod_venc = venc.cod_venc
join divisao divi on emp.lotacao_div = divi.cod_divisao
join departamento dep on divi.cod_dep = dep.cod_dep
group by dep.nome
