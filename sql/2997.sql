...
/*  Solution */
/*
select d2.nome as nome_departamento, 
       d3.nome as divisao,
	   e2.nome as nome_empregado, 
	   bruto.salario_bruto, 
	   desconto.valor_desconto as total_descontos, 
	   (bruto.salario_bruto - desconto.valor_desconto) as salario_liquido
from 
    (select e.matr, coalesce(sum(v.valor), 0) as salario_bruto
    from empregado e
    left join emp_venc ev on e.matr = ev.matr
    left join vencimento v on ev.cod_venc = v.cod_venc
    group by e.matr) as bruto
join
    (select e.matr, coalesce(sum(d.valor), 0) AS valor_desconto
    from empregado e
    left join emp_desc ed on e.matr = ed.matr
    left join desconto d on ed.cod_desc = d.cod_desc
    group by e.matr) as desconto
on bruto.matr = desconto.matr
join empregado e2 on bruto.matr = e2.matr
left join departamento d2 on e2.gerencia_cod_dep = d2.cod_dep
left join divisao d3 on e2.lotacao_div = d3.cod_divisao
order by d2.nome, d3.nome, salario_liquido desc
*/
